  ...
  const handleFinish = async () => {
    try {
      ...
      if (error instanceof ApiError) {
        ...
        setErrorText('No se pudo finalizar la sesión.');
      }
    } finally {
      setSubmitting(false);
    }
      ...
      <View style={s.body}>
        ...
        <View style={s.card}>
          ...
          <Text style={s.cardBody}>
            Puedes continuar la sesión o finalizarla y revisar el resumen.
          </Text>
          ...
          <View style={s.infoRow}>
            ...
            <View style={s.stateBadge}>
              ...
              <Text style={s.stateTxt}>En curso</Text>
            </View>
          </View>
        </View>

        <View style={s.actions}>
          <Pressable style={({ pressed }) => [s.btn, s.btnPrimary, pressed && s.btnPressed]} onPress={() => navigation.goBack()}>
            <Text style={s.btnTxtLight}>▶ Continuar sesión</Text>
          </Pressable>

          <Pressable style={({ pressed }) => [s.btn, s.btnDanger, pressed && s.btnPressed]} onPress={() => void handleFinish()} disabled={submitting}>
            {submitting ? <ActivityIndicator size="small" color={C.white} /> : <Text style={s.btnTxtLight}>■ Finalizar y ver resumen</Text>}
          </Pressable>

          <Pressable style={({ pressed }) => [s.btn, s.btnOutline, pressed && s.btnPressed]} onPress={() => navigation.goBack()}>
...
const s = StyleSheet.create({
  ...
  topBar: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: C.primary, paddingHorizontal: 14, paddingVertical: 10, gap: 10, ...elev(4),
  },
  ...
  pauseIndicator: {
    flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: `${C.warning}30`, borderWidth: 1, borderColor: `${C.warning}66`, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20,
  },
  ...
  iconWrap: {
    width: 72, height: 72, borderRadius: 24, backgroundColor: `${C.warning}18`, borderWidth: 2, borderColor: `${C.warning}44`, justifyContent: 'center', alignItems: 'center',
  },
  iconText: { fontSize: 32 },
  card: {
    width: '100%', maxWidth: 420, backgroundColor: C.bgSurface, borderRadius: 16, borderWidth: 1, borderColor: C.border, padding: 20, gap: 10, ...elev(3),
  },
  ...
  errorText: { color: C.error, fontSize: 13, fontWeight: '600', textAlign: 'center' },
  ...
  btn: { width: '100%', paddingVertical: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
